...
if ($result['success'] && isset($result['data']['d'])) {
    $purchaseInvoices = $result['data']['d'];
    
    // Sort purchase invoices by ID descending (newest first)
    usort($purchaseInvoices, function($a, $b) {
...
<head>
    ...
    <title>Daftar Purchase Invoice - Nuansa</title>
...
<body class="bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h1 class="text-3xl font-bold text-gray-900">Daftar Purchase Invoice</h1>
                </div>
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow p-6">
            <h2 class="text-xl font-semibold mb-4">Data Purchase Invoice</h2>
            
            <?php if (!empty($purchaseInvoices)): ?>
                <div class="mb-4">
                    <p class="text-sm text-gray-600">
                        <i class="fas fa-info-circle mr-1"></i>
                        Menampilkan <?php echo count($purchaseInvoices); ?> purchase invoice.
                    </p>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Age</th>
                            ...
                            <?php foreach ($purchaseInvoices as $invoice): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo htmlspecialchars($invoice['id'] ?? 'N/A'); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo htmlspecialchars($invoice['number'] ?? 'N/A'); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <span class="px-2 py-1 text-xs font-medium rounded-full 
                                            <?php 
                                            $status = $invoice['statusName'] ?? '';
                                            if (stripos($status, 'paid') !== false || stripos($status, 'lunas') !== false) {
                                                echo 'bg-green-100 text-green-800';
                                            } elseif (stripos($status, 'partial') !== false || stripos($status, 'sebagian') !== false) {
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                        <span class="px-2 py-1 text-xs font-medium rounded-full 
                                            <?php 
                                            $percentPaid = $invoice['percentPaid'] ?? 0;
                                            if ($percentPaid == 100) {
                                                echo 'bg-green-100 text-green-800';
                                            ...
                                            ?>">
                                            <?php echo htmlspecialchars($percentPaid); ?>%
                                        </span>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                        <?php 
                                        $age = $invoice['age'] ?? null;
                                        if ($age !== null) {
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="detail.php?id=<?php echo $invoice['id']; ?>" 
            ...
            <?php else: ?>
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-600">Tidak ada data purchase invoice.</p>
            ...
            <div class="mt-6 bg-gray-50 rounded-lg p-4">
                ...
                <div class="text-sm">
                    <p><strong>Data Source:</strong> Purchase Invoice List API (/list.do)</p>
                    <p><strong>List API Success:</strong> <?php echo isset($result) && $result['success'] ? 'Yes' : 'No'; ?></p>
                    <p><strong>Purchase Invoices Found:</strong> <?php echo count($purchaseInvoices); ?></p>
                    <?php if (isset($result['message']) && !$result['success']): ?>
