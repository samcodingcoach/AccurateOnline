...
$api = new AccurateAPI();
$result = $api->getShipmentList();
$shipments = [];

if ($result['success'] && isset($result['data']['d'])) {
    $shipments = $result['data']['d'];
    
    // Sort shipments by ID ascending
...
function getShipmentStatus($shipment) {
    $suspended = $shipment['suspended'] ?? false;
...
function getStatusBadge($shipment) {
    $isActive = getShipmentStatus($shipment);
    if ($isActive) {
...
<head>
    ...
    <title>Daftar Shipment - Nuansa</title>
...
<body class="bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h1 class="text-3xl font-bold text-gray-900">Daftar Shipment</h1>
                </div>
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow p-6">
            <h2 class="text-xl font-semibold mb-4">Data Shipment</h2>
            
            <?php if (!empty($shipments)): ?>
                <div class="mb-4">
                    <p class="text-sm text-gray-600">
                        <i class="fas fa-info-circle mr-1"></i>
                        Menampilkan <?php echo count($shipments); ?> shipment.
                    </p>
                </div>
                            ...
                            <?php foreach ($shipments as $shipment): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo htmlspecialchars($shipment['id'] ?? 'N/A'); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        <?php echo htmlspecialchars($shipment['name'] ?? 'N/A'); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo htmlspecialchars($shipment['picPhoneNumber'] ?? 'N/A'); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo getStatusBadge($shipment); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="detail.php?id=<?php echo $shipment['id']; ?>" class="text-blue-600 hover:text-blue-900">
            ...
            <?php else: ?>
                <p class="text-gray-600">Tidak ada data shipment.</p>
            <?php endif; ?>
        </div>
    </main>
</body>
